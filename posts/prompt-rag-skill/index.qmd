...
description-meta: "以 EndoViHo-RAG 为例，从模型调用、外部证据与可复用工作流三个层面，区分 Prompt、RAG 和 Skill。"
open-graph:
  description: "以 EndoViHo-RAG 为例，从模型调用、外部证据与可复用工作流三个层面，区分 Prompt、RAG 和 Skill。"
...
EndoViHo-RAG 是我正在搭建的一个面向内源性病毒元件（endogenous viral element，EVE）的可审计 Hybrid RAG。它希望把仅在单个基因组组装（Assembly）范围内定义的 EVE 记录、Assembly 来源分类指派与文献证据放进同一套查询流程中。

最近学习设计 AI 工具时，我反复遇到 Prompt、RAG 和 Skill 这三个词。它们经常被放在一起讨论，但真正开始搭系统后，我发现有必要把它们分开想清楚。
...
以下仍是面向未来系统的示意 Prompt，并非当前可运行的功能。

```text
只根据系统返回的记录回答问题。
列出 Assembly 来源分类群、Assembly 编号、当前 DatasetRelease 明确纳入的 assembly-local EVE 位点（loci）数量和支持证据。
...
这里的检索不一定依赖向量数据库。它可以来自 SQL、全文检索、知识图谱、网页、API、实验室内部数据，也可以把几种方式组合起来。这里列举的是 RAG 的一般可能性；EndoViHo-RAG 当前规划使用固定文献语料，不进行实时网页搜索。
...
例如，将来可以为 EndoViHo-RAG 设计一个“查询 DatasetRelease 中 EVE loci 及其 Assembly 来源分类指派”的 Skill。它可以先把用户输入映射到受控的分类名称，再选择结构化查询、文献检索或混合路线，随后按照已确认的位点身份规则汇总、校验结果并输出表格。任何关键步骤失败时，流程都应明确停止，而不是让模型凭常识补齐缺失结果。

截至 Milestone 0，EndoViHo-RAG 还没有完成领域数据表、数据导入流程、检索路线或可调用的 Skill。位点身份、去重和 DatasetRelease 纳入规则也尚未冻结。因此，未来流程不能把结果写成“唯一病毒数量”；只有等这些规则和记录具备后，才能报告当前 DatasetRelease 明确纳入的 assembly-local EVE loci。
...
EndoViHo-RAG 目前仍处于 Milestone 0，领域数据、检索路线和可调用的 Skill 尚未完成。下面不是现有功能演示，而是一个用于说明职责分工的设计例子。

假设未来已经导入并发布了相应数据，用户提出：

> 在当前发布的数据中，哪些来源于双壳类（Bivalvia）的基因组 Assembly 收录了与 Orthopolintovirales 相关的 EVE 位点？当前 DatasetRelease 为每个 Assembly 明确纳入了多少个 assembly-local 位点？
...
- 只根据系统返回的记录作答；
- 输出 Assembly 来源分类群、Assembly 编号、当前 DatasetRelease 明确纳入的 assembly-local EVE 位点数量和证据来源；
- 保留数据集与文献版本；
- 不把“没有记录”解释为“生物学上不存在”。
